...
export default class {
  ...
  // modification
  // gestion de l'état d'ouverture section par section dans handleShowTickets
  handleShowTickets(e, bills, index) {
    this.openSections = this.openSections || {};

    const isOpen = this.openSections[index] || false;
    ...
    if (!isOpen) {
      ...
      this.openSections[index] = true;
    } else {
      $(`#arrow-icon${index}`).css({ transform: "rotate(90deg)" });
      $(`#status-bills-container${index}`).html("");

      this.openSections[index] = false;
    }
